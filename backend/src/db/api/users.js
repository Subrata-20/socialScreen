...
module.exports = class UsersDBApi {
  ...
  static async findBy(where, options) {
    ...
    output.achievements_child = await users.getAchievements_child({
      transaction,
    });

    output.rewards_child = await users.getRewards_child({
      transaction,
    });

    output.screen_time_recommendations_child =
      await users.getScreen_time_recommendations_child({
        transaction,
      });

    output.sentiment_check_ins_child = await users.getSentiment_check_ins_child(
      {
        transaction,
      },
    );
